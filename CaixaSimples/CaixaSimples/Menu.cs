...
namespace CaixaSimples
{
    public partial class frmMenu : Form
    {
        ...
        /*
        private void imprimi_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs este)
        {
            
            carregaTblMensalidade();
            Bitmap bm = new Bitmap(dgvImprimi.Width, dgvImprimi.Height);
            dgvImprimi.DrawToBitmap(bm, new Rectangle(0,0, dgvImprimi.Width,dgvImprimi.Height));

            
            string resumo = "Dinheiro : \nStaídas : \nDébito : \nCrédito :\nTotal Cartão : \nTotal Entradas : \nTotal Produtos : \nTotal Espécie : \nFundo de Caixa : \nTotal Depósito : \nValor Apresentado : ";
            string resumo2 = StatusDoCaixa.dinheiro.ToString("C") + "\n" + StatusDoCaixa.saida.ToString("C") + "\n" + StatusDoCaixa.debito.ToString("C") + "\n" + StatusDoCaixa.credito.ToString("C") + "\n" + StatusDoCaixa.ttcartao.ToString("C") + "\n" + StatusDoCaixa.ttentrada.ToString("C") + "\n" + StatusDoCaixa.ttprodutos.ToString("C") + "\n" + StatusDoCaixa.ttespecie.ToString("C") + "\n" + StatusDoCaixa.fdCaixa.ToString("C") + "\n" + StatusDoCaixa.ttDeposito.ToString("C") + "\n" + StatusDoCaixa.valoApresentado.ToString("C") + "\n";
            string mensalidade = "";
            foreach (DataRow linha in tblRMensalidade.Rows)
            {
                mensalidade += linha[0].ToString() + "\n";
                //mensalidade += linha[1].ToString() + "\t";
                //mensalidade += linha[2].ToString() + "\t";
                //mensalidade += linha[3].ToString() + "\t";
                //mensalidade += linha[4].ToString() + "\t";
                //mensalidade += Convert.ToDouble(linha[5].ToString()).ToString("C") +"\n";
            }
            
            este.Graphics.DrawString("Centro Educacional Shekinah", new Font("Arial", 20, FontStyle.Bold), Brushes.Black, new PointF(210, 30));
            este.Graphics.DrawString("Data : " + StatusDoCaixa.data.Date.ToString("dd/MM/yyyy"), new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new PointF(325, 60));
            este.Graphics.DrawString(resumo, new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new PointF(100, 90));
            este.Graphics.DrawString(resumo2, new Font("Arial", 16, FontStyle.Regular), Brushes.Black, new PointF(600, 90));
            este.Graphics.DrawString(mensalidade, new Font("Arial", 10, FontStyle.Regular), Brushes.Black, new PointF(20, 440));
            este.Graphics.DrawImage(bm, 0, 0);
